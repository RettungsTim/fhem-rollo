...
my $version = "1.202-Rollosteuerung-Cluni";
...
#	$attr{$name}{"devStateIcon"}; 													#wird jetzt abhängig von Attribut type definiert!
	...
	$attr{$name}{"Auto_Pos_Befehl"} = "pct";										# Befehl, mit dem der Rollladen gefahren wird (bei Homatic 'pct' (default), bei ROLLO-Modul 'pct')
	...
	$attr{$name}{"Auto_Frostschutz"} = "nein";
	$attr{$name}{"Auto_Lueften_Pos"} = 30;											# pct des Rollladen, fürs automatische Lüften bei gekipptem Fenster
	$attr{$name}{"Auto_Geoeffnet_Pos"} = 80;										# pct des Rollladen, fürs automatische Öffnen bei geöffnetem Fenster (z.B. Drehgriffkontakt)
	$attr{$name}{"Auto_offen_Pos"} = 100;											# pct des Rollladen, wenn morgens automatisch geöffnet wird
	...
	$attr{$name}{"Auto_Luft_Fenster_offen"} = "ja";									# auf Lüften, wenn Fenster gekippt/geöffnet wird und aktuelle pct unterhalb der Lüften-pct
	...
	$attr{$name}{"Auto_offen_Pos"} = 20;											# pct des Rollladen nach dem automatische Öffnen am Morgen
	$attr{$name}{"Auto_geschlossen_Pos"} = 0;										# pct, auf den der Rollladen beim Schließen fahren soll
	$attr{$name}{"Auto_Pos_nach_KomfortOeffnen"} = -1;								# Angabe der pct nach dem automatischen Öffnen über Fensterkontakt. -1 = vorherige pct anfahren -2 = geöffnet-pct behalten
	...
	$attr{$name}{"Auto_Abschattung_Pos"} = 30;										# pct des Rollladen, für die automatische Abschattung
	$attr{$name}{"Auto_Himmelsrichtung"} = 178;										# pct in Grad, auf der das Fenster liegt - genau Osten wäre 90, Süden 180 und Westen 270 - wird bei der Abschattung berücksichtigt
	...
	$attr{$name}{"Auto_Abschattung"} = "ja";										# Modus der automatischen Abschattung (ja,nein,verspaetet,bei_Abwesenheit,bei_Anwesenheit)
	...
	$attr{$name}{"Auto_Abschattung_Schwelle_sonnig"} = 4000;						# obere Schwelle für die Helligkeit
	$attr{$name}{"Auto_Abschattung_Schwelle_wolkig"} = 3000;						# untere Schwelle für die Helligkeit
	...
	$attr{$name}{"Auto_Abschattung_Pos_nach_Abschattung"} = -1;						# pct des Rollladen, wenn Abschatten beendet wird. Bei -1 wird die vorherige pct angefahren
	...
	$attr{$name}{"userReadings"} = "pct {100-ReadingsNum($name,'pct',0)}";
	$attr{$name}{"subType"} = "blindActuator";
	$attr{$name}{"type"} = "HomeKit";
	$attr{$name}{"event-on-change-reading"} = "state";
	$attr{$name}{"commandDown"} = "set EG_tz_RO_mi_RUNTER on";
	$attr{$name}{"commandDown2"} = "set EG_tz_RO_mi_HOCH off";
	$attr{$name}{"commandStopDown"} = "set EG_tz_RO_mi_RUNTER off";
	$attr{$name}{"commandStopUp"} = "set EG_tz_RO_mi_HOCH off";
	$attr{$name}{"commandUp"} = "set EG_tz_RO_mi_HOCH on";
	$attr{$name}{"commandUp2"} = "set EG_tz_RO_mi_RUNTER off";
  return undef;
}
